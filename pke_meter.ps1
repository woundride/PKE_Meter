...
Write-Host '                                                                             Version 1.14'
Write-Host '                                                                Release date : 2023/10/23'
...
2{
  ...
  while ( $user_extract -lt 1 -or $user_extract -gt 21) {
	...
	Write-Host '2 - Administrator Account(s) [belonging to an administrator group]'
	...
	1{
		...
		foreach( $acc in $orphanList ){
			if (( Read-Host 'Reset security descriptor for account '$($dn)'? - Administrator rights needed - Answer y' ) -eq 'y' ){
				ResetAccount $dn
			}
		}
