...
Write-Host '                                                                Release date : 2021/03/05'
...
1{
	...
	16{
		...
		Write-Host ''
		Get-adobject -ldapfilter '(cn=*cnf:*)' ; Get-adobject -ldapfilter '(sAMAccountName=$duplicate)' | Select SamAccountName, LastLogonDate, OperatingSystem, OperatingSystemVersion, CanonicalName | Export-Csv C:\Users\$user\Desktop\duplicated_computers_accounts.csv
		Write-Host 'Exported file :'
		Write-Host ''
...
2{
  ...
  while ( $user_extract -lt 1 -or $user_extract -gt 13) {
	Write-Host '      ////^\\\\'
	Write-Host '      | ^   ^ |'
	Write-Host '     @ (o) (o) @'
	Write-Host '      |   <   |'
	...
	Write-Host -ForegroundColor Red '8 - All users whose password never expire'
	Write-Host -ForegroundColor Red '9 - Users who cannot change their password'
	Write-Host '10 - Users with mailbox'
	Write-Host '11 - Users without mailbox'
	Write-Host -ForegroundColor Cyan '12 - Password last set for all users accounts (enabled)'
	Write-Host ''
	Write-Host -ForegroundColor Yellow '13 - Return'
	Write-Host ''
	Write-Host '-----------------------------------------------------------------------------------------'
	Write-Host ''
	...
	8{
		...
		Search-ADAccount -PasswordNeverExpires -UsersOnly -ResultPageSize 2000 -resultSetSize $null | Select-Object Name, SamAccountName, DistinguishedName | Export-CSV c:\Users\$user\Desktop\all_users_whose_password_never_expire.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
	...
	9{
		...
		Get-ADUser -filter * -properties * | where { $_.CannotChangePassword -eq 'true' } | Select-Object Name, SamAccountName, LastLogonDate, DistinguishedName | Export-CSV c:\Users\$user\Desktop\users_who_cannot_change_their_password.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
	...
	10{
		...
		Get-ADUser -filter 'mail -like "*"' -properties * | Select-Object Name, SamAccountName, LastLogonDate, DistinguishedName, Mail | Export-CSV c:\Users\$user\Desktop\users_with_mailbox.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
	...
	11{
		...
		Get-ADUser -filter 'mail -notlike "*"' -properties * | Select-Object Name, SamAccountName, LastLogonDate, DistinguishedName | Export-CSV c:\Users\$user\Desktop\users_without_mailbox.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
	...
	12{
		...
		Get-ADUser -filter {ObjectClass -like 'user' -and Enabled -eq 'true'} -properties * | Select-Object Name, SamAccountName, PasswordLastSet, DistinguishedName | Export-CSV c:\Users\$user\Desktop\password_last_set_for_all_users.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
...
3{
  ...
  while ( $group_extract -lt 1 -or $group_extract -gt 4) {
Write-Host '                                 ___                           '
...
Write-Host '                                                    |_|        '
	...
	Write-Host ''
	Write-Host -ForegroundColor Yellow '4 - Return'
	Write-Host ''
	Write-Host '-----------------------------------------------------------------------------------------'
	Write-Host ''
	...
	1{
		...
		Get-ADGroup -Filter * | Select-Object samAccountName, DistinguishedName | Export-CSV c:\Users\$user\Desktop\all_groups.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
	...
	2{
		...
		Get-AdGroupMember -Identity $group_name | Select-Object Name, SamAccountName, DistinguishedName | Export-CSV c:\Users\$user\Desktop\$group_name.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
	...
	3{
		...
		Get-ADPrincipalGroupMembership -Identity $object_name | Select-Object Name, SamAccountName, DistinguishedName | Export-CSV c:\Users\$user\Desktop\$object_name.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
