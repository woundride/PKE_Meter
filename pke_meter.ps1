...
Write-Host '                                                                Release date : 2021/05/29'
...
2{
  ...
  while ( $user_extract -lt 1 -or $user_extract -gt 15) {
	Write-Host '      ////^\\\\'
	Write-Host '      | ^   ^ |'
	Write-Host '     @ (o) (o) @'
	Write-Host '      |   <   |'
	...
	Write-Host -ForegroundColor Cyan '13 - Password last set for a specific user account'
	Write-Host -ForegroundColor Cyan '14 - Last logon date for a specific user account'
	Write-Host ''
	Write-Host -ForegroundColor Yellow '15 - Return'
	Write-Host ''
	Write-Host '-----------------------------------------------------------------------------------------'
	Write-Host ''
	...
	5{
		...
		Search-ADAccount -UsersOnly -AccountInActive -TimeSpan 90:00:00:00 -ResultPageSize 2000 -ResultSetSize $null | ?{$_.Enabled -eq $True} | Select-Object Name, SamAccountName, LastLogonDate, DistinguishedName | Export-CSV -Encoding UTF8 c:\Users\$user\Desktop\inactive_users.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
		...
		Write-Host ''
		if (( Read-Host 'Do you want to disable inactive accounts ? - Administrator rights needed - Answer "sure" to validate' ) -eq 'sure' ){
				Search-ADAccount -UsersOnly -AccountInActive -TimeSpan 90:00:00:00 -ResultPageSize 2000 -ResultSetSize $null | ?{$_.Enabled -eq $True} | Select-Object SamAccountName | Export-CSV -Encoding UTF8 c:\Users\$user\Desktop\inactive_users_to_disable.csv -NoTypeInformation
	...
	6{
		...
		Get-ADUser -filter {ObjectClass -like 'user' -and Enabled -eq 'true'} -properties * | Select-Object Name, SamAccountName, LastLogonDate, DistinguishedName | Export-CSV -Encoding UTF8 c:\Users\$user\Desktop\all_users.csv -NoTypeInformation
		Write-Host ''
		Write-Host 'Exported file :'
		Write-Host ''
